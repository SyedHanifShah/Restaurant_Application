...
import com.corn.cornpos.ui.screen.dinein.DineInEvents
...
class DineInViewModel @Inject constructor(
    ...
    fun onEvent(event: DineInEvents) {
        when (event) {
            is DineInEvents.OnNewOrderClick -> {
                viewModelScope.launch {
                    dataStore.saveServiceTypeID(1)
                    dataStore.savedTakeAwayTitle("Dine In")
                }

                viewModelScope.launch {
                    dataStore.saveCanDineIn(true)
                }
